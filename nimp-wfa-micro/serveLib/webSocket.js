...
const { messageCenter } = require("./messageCenter.js");

const ModeCode = {
  //websocket消息类型
  MSG: "message", //普通消息
  HEART_BEAT: "heart_beat", //心跳
  HTML: "html", // 写入 html 模板
  EXEC_TS: "execTs", // 写入 TS 模板
  MODEL: "model", // 写入 TS 模板
};
...
class MyWebSocket extends WebSocket {
  ...
  messageHandler(e) {
    ...
    console.log("data", data.ModeCode);
    const writeUrl = "./src/app/ng-bas-sys/wferesource/previews";
    switch (data.ModeCode) {
      ...
      case ModeCode.HTML:
        fs.writeFileSync(
          path.resolve(__dirname, "..", `${writeUrl}/previews.component.html`),
          data.data
        );
        break;
      case ModeCode.EXEC_TS:
        fs.writeFileSync(
          path.resolve(__dirname, "..", `${writeUrl}/previews.component.ts`),
          data.data
        );
        break;
      case ModeCode.MODEL:
        fs.writeFileSync(
          path.resolve(__dirname, "..", `${writeUrl}/model/request-param.model.ts`),
          data.data
        );
        break;
      default:
        console.log("收到未知消息" + data.msg);
    }
  }
